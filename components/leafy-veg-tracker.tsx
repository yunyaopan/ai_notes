...
export function LeafyVegTracker() {
  ...
  const loadData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('[LeafyVegTracker] State updated - weekTotal:', data.weekTotal || 0);
      } else {
        console.error('[LeafyVegTracker] Load data failed:', response.status, await response.text());
      }
    } catch (error) {
  ...
  const handleSaveToday = async () => {
    ...
    console.log('[LeafyVegTracker] handleSaveToday called with grams:', grams, 'today:', today);
    ...
    setSaving(true);
    try {
      console.log('[LeafyVegTracker] Sending POST request with:', { date: today, grams });
      const response = await fetch('/api/leafy-veg', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log('[LeafyVegTracker] Setting weekTotal to:', data.weekTotal || 0);
        setWeekTotal(data.weekTotal || 0);
        ...
        if (reloadResponse.ok) {
          ...
          console.log('[LeafyVegTracker] Setting weekTotal from reload to:', reloadData.weekTotal || 0);
          ...
          console.error('[LeafyVegTracker] Reload failed:', reloadResponse.status);
        }
      } else {
  ...
  const handleSaveEdit = async (entryId: number, date: string) => {
    ...
    console.log('[LeafyVegTracker] handleSaveEdit called with:', { entryId, date, grams });
    if (isNaN(grams) || grams < 0) {
      alert('Please enter a valid number of grams (0 or greater)');
      return;
    }
    ...
    try {
      console.log('[LeafyVegTracker] Sending POST request for edit with:', { date, grams });
      const response = await fetch('/api/leafy-veg', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {/* Recent Entries */}
        <div className="space-y-2">
          <Label className="text-base font-semibold">Recent Entries (Last 10)</Label>
          {entries.length === 0 ? (
            ...
            <div className="space-y-2">
              {entries.map((entry) => {
                ...
                const dateFormatted = entryDate.toLocaleDateString('en-US', {
                  ...
                  month: 'short',
                  day: 'numeric',
                  year: entryDate.getFullYear() !== new Date().getFullYear() 
                    ? 'numeric' 
                    : undefined
                });
